...
function tick(s:RiftState,input:RiftInput,dt:number,power:string,evolutionTier:number){
 ...
 if(input.power&&s.powerCooldown===0){input.power=false;s.powerCooldown=5-Math.max(0,Math.min(2,evolutionTier-1));s.powerFlash=.5;s.powerUses++;const kind=powerKind(power);if(kind==='shield')s.invulnerable=2.5;else if(kind==='heal')s.health=Math.min(100,s.health+25);else if(kind==='dash'){s.invulnerable=1;for(let i=0;i<15;i++)move(s.player,(x||(!y?s.facing:0))/norm*7,y/norm*7);}else for(const e of s.enemies)if(riftDistance(e,s.player)<220&&clearSight(s.player,e))hitEnemy(s,e,5);}
...
export function stepRift(s:RiftState,input:RiftInput,delta:number,power:string,evolutionTier=1){if(s.status!=='playing'||!Number.isFinite(delta)||delta<=0)return;let left=Math.min(delta,.05);while(left>1e-6&&s.status==='playing'){const dt=Math.min(left,1/120);tick(s,input,dt,power,evolutionTier);left-=dt;}}
