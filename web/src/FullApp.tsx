...
const JourneyMenu = lazy(() => import("./world/JourneyMenu").then(module => ({ default: module.JourneyMenu })));
...
import "./world/connected-polish.css";
const Arcade = lazy(() => import("./world/Arcade").then(module => ({ default: module.Arcade })));
const RobotHome = lazy(() => import("./world/RobotHome").then(module => ({ default: module.RobotHome })));
...
export default function FullApp() {
 ...
 useEffect(() => {
  ...
  window.addEventListener("keydown", rememberKeyboard, true); window.addEventListener("pointerdown", rememberPointer, true);
  return () => { window.removeEventListener("keydown", rememberKeyboard, true); window.removeEventListener("pointerdown", rememberPointer, true); };
 }, []);
 useLayoutEffect(() => {
  ...
  const focusAfterKeyUp = () => { scheduleFocus(); pendingTitleFocus.current = null; };
  window.addEventListener("keyup", focusAfterKeyUp, { once: true });
  return () => { window.removeEventListener("keyup", focusAfterKeyUp); window.cancelAnimationFrame(firstFrame); window.cancelAnimationFrame(secondFrame); };
 ...
 const open = (sectionId: SectionId) => {
  ...
  commitProfile(current => ({ ...current, selectedSection: sectionId, sectionVisits: { ...current.sectionVisits, [sectionId]: Number(current.sectionVisits[sectionId] ?? 0) + 1 } }));
  announce(`${tr(ui.openDestination, profile.language)}: ${tr(section.name, profile.language)}`);
 };
 const beginStarBridge = () => {
  const sectionId: SectionId = "robo-lab"; presentSection();
  commitProfile(current => { const next = applyStarBridgeEvent(current, { type: "REVEAL_BRIDGE" }); return { ...next, selectedSection: sectionId, sectionVisits: { ...next.sectionVisits, [sectionId]: Number(next.sectionVisits[sectionId] ?? 0) + 1 } }; });
  announce(profile.language === "es-MX" ? "Aventura iniciada: prepara a BoltBot en el Laboratorio de Robots" : "Adventure started: prepare BoltBot in Robo Lab");
 };
 const advanceStarBridge = (event: StarBridgeEvent) => {
  ...
  const message = messages[event.type]; if (message) announce(message[profile.language]);
 };
 const continueAdventure = () => {
  const next = nextAdventure(profile.adventures.starBridge.step,profile.language);
  if(next.action==='begin') beginStarBridge();
  else if(next.action==='bridge') {open('world-map');setBridgeRequested(true);}
  else open(next.action==='home'?'robot-home':'robo-lab');
 };
 const openCreation = (kind:'art'|'story',id:string) => {
  open(kind==='art'?'art-studio':'story-castle');setCreationIntent({kind,id});
 };
 const page = (() => {
  ...
  switch (profile.selectedSection) {
   ...
   case "becca-corner": return <Suspense fallback={<div className="fw-empty" role="status">{profile.language === "es-MX" ? "Abriendo el taller mágico…" : "Opening the magic workshop…"}</div>}><BeccaCorner language={profile.language} /></Suspense>;
   case "cousins-adventure": return <Suspense fallback={<div className="fw-empty" role="status">{profile.language === "es-MX" ? "Abriendo el mapa de aventuras…" : "Opening the adventure map…"}</div>}><CousinsAdventure language={profile.language} /></Suspense>;
   case "monster-lab": return <MonsterLab {...props} />;
   case "monster-habitats": return <MonsterHabitats {...props} />;
   case "art-studio": return <ArtStudio {...props} initialArtworkId={creationIntent?.kind==='art'?creationIntent.id:undefined} openHome={()=>open('robot-home')} />;
   case "story-castle": return <StoryCastle {...props} initialStoryId={creationIntent?.kind==='story'?creationIntent.id:undefined} openHome={()=>open('robot-home')} />;
   ...
   case "robot-home": return <RobotHome {...props} open={open} beginStarBridge={beginStarBridge} continueAdventure={continueAdventure} openArtwork={id=>openCreation('art',id)} openStory={id=>openCreation('story',id)} />;
   ...
   default: return <WorldMap profile={profile} open={open} beginStarBridge={beginStarBridge} advanceStarBridge={advanceStarBridge} onCreate={()=>setMenu('create')} initialBridgeOpen={bridgeRequested} />;
  }
 })();
 ...
 return <div className="fw-app" data-active-section={profile.selectedSection} data-play-layout={playLayout}>
  ...
  {playLayout && <JourneyNavigation compact profile={profile} open={open} showMenu={setMenu} />}
  ...
  <BottomNavigation profile={profile} open={open} showMenu={setMenu} />
  {menu && <Suspense fallback={<p role="status">{profile.language==='es-MX'?'Abriendo menú…':'Opening menu…'}</p>}><JourneyMenu kind={menu} language={profile.language} open={open} close={()=>setMenu(null)} /></Suspense>}
 </div>;
}
