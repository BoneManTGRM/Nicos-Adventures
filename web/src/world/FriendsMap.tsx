...
export function FriendsMap({ profile, update, announce, close }: { profile: LocalProfile; update: UpdateProfile; announce: Announce; close: () => void }) {
  ...
  return <section ref={region} className="friends-map" aria-label={es ? 'Nico y sus amigos' : 'Nico & Friends'} data-map-status={view.status} data-map-mode={view.mode} data-map-progress={view.count} data-map-completed={view.completed}>
    ...
    <div className="friends-map__status"><span>{es ? instructions.es : instructions.en} <b>{view.count}/{view.total}</b></span><span>★ {view.score}</span><button type="button" onClick={() => { pause(); setMenu(true); }}>{es ? 'Lugares y modos' : 'Places & modes'}</button></div>
    ...
    {menu && <div className="friends-map__scrim"><div ref={dialog} className="friends-map__menu" role="dialog" aria-modal="true" aria-label={es ? 'Lugares y modos' : 'Places & modes'}>
      ...
      <div className="friends-map__destinations">{(view.mode === 'stars' ? TOKENS : STOPS).map((point, i) => <button type="button" key={point.id} data-map-destination={point.id} onClick={() => walk(point)}><b>{i + 1}</b><span>{'en' in point ? (es ? point.es : point.en) : `${es ? 'Estrella' : 'Star'} ${i + 1}`}</span><span>{new Set<string>(view.mode === 'stars' ? s.collected : s.completed).has(point.id) ? '✓' : '→'}</span></button>)}</div>
