...
export function RobotHome({ profile, update, announce }: { profile: LocalProfile; update: UpdateProfile; announce: Announce }) {
  ...
  const toggleDecoration = (item: string) => {
    ...
    announce(language === "es-MX" ? `${optionLabel(item, language)} ${selected ? "quitada" : "agregada"}.` : `${item} ${selected ? "removed" : "added"}.`);
  };
  const claimGoal = (goal: RoomGoal) => {
    const missionId = roomGoalId(goal.id);
    if (!goal.ready || hasCompleted(profile, missionId)) return;
    const completion = completeOnce(profile, missionId, goal.reward); update(completion.profile);
  ...
  return <div className="robot-home-system">
    ...
    <section className="room-goal-panel" aria-labelledby="room-goals-heading"><header><div><small>{language === "es-MX" ? "Recompensas únicas" : "One-time rewards"}</small><h2 id="room-goals-heading">{language === "es-MX" ? "Metas de la Casa Robot" : "Robot Home goals"}</h2></div></header><div className="room-goal-grid">{roomGoals.map(goal => {
      const completed = hasCompleted(profile, roomGoalId(goal.id));
      return <article className={`${goal.ready ? "ready" : ""} ${completed ? "completed" : ""}`.trim()} key={goal.id}><span aria-hidden="true">{goal.emoji}</span><div><h3>{goal.title[language]}</h3><small>⭐ {goal.reward}</small></div><button type="button" disabled={!goal.ready || completed} onClick={() => claimGoal(goal)}>{completed ? (language === "es-MX" ? "Completada" : "Completed") : goal.ready ? (language === "es-MX" ? "Reclamar" : "Claim") : (language === "es-MX" ? "En progreso" : "In progress")}</button></article>;
    })}</div></section>
  </div>;
}
