...
const storyScenes: Record<CousinsDestinationId, [string, string]> = {
  "rainbow-forest": [rainbowOne, rainbowTwo],
  "dino-valley": [dinoOne, dinoTwo],
  "star-harbor": [starOne, starTwo],
  "sky-castle": [castleOne, castleTwo],
  "moon-garden": [moonOne, moonTwo],
};
...
export function CousinsAdventure({ language }: { language: Language }) {
  ...
  const choiceResult = interactiveChoices[language].find((item) => item.id === storyChoice)?.result;
  const activeScene = storyScenes[destinationId][pageIndex < 2 ? 0 : 1];
    ...
    <div className="cousins-adventure">
      <section className="cousins-hero">
        <div className="cousins-hero__copy">
          ...
          <p>{language === "es-MX"
            ? "Toca un libro del mapa. Elige quién encuentra la primera pista y crea un cuento ilustrado de cinco páginas."
            : "Tap a book on the map. Choose who finds the first clue, then create a five-page illustrated story."}</p>
          ...
          <fieldset>
            ...
            <div>
              {(Object.keys(THEME_LABELS) as AdventureTheme[]).map((item) => (
                <button type="button" key={item} className={theme === item ? "is-active" : ""} aria-pressed={theme === item} onClick={() => { setTheme(item); setPageIndex(0); setStoryChoice(null); }}>
          ...
          <fieldset>
            ...
            <div>
              {(Object.keys(leadLabels) as CousinLead[]).map((item) => (
                <button type="button" key={item} className={lead === item ? "is-active" : ""} aria-pressed={lead === item} onClick={() => { setLead(item); setPageIndex(0); setStoryChoice(null); }}>
        ...
        <article className="cousins-storybook" aria-live="polite">
          ...
          <div className="cousins-storybook__page">
            ...
            {pageIndex === 1 && !storyChoice && (
              <div className="storybook-choice" role="group" aria-label={language === "es-MX" ? "Elige cómo buscar la pista" : "Choose how to find the clue"}>
                ...
                {interactiveChoices[language].map((item) => (
                  <button type="button" key={item.id} onClick={() => { setStoryChoice(item.id); setPageIndex(2); }}>
                    <span aria-hidden="true">{item.icon}</span>{item.label}
                  </button>
                ))}
              </div>
            )}
            {pageIndex >= 2 && choiceResult && <div className="storybook-result">✨ {choiceResult}</div>}
            ...
            <div className="storybook-controls">
              ...
              <button type="button" disabled={pageIndex === 1 && !storyChoice} onClick={() => setPageIndex((current) => current === pages.length - 1 ? 0 : current + 1)}>
