(() => {
  ...
  style.textContent = `.dinosaur-paleoart{margin:0 0 1.25rem;border-radius:1.5rem;overflow:hidden;border:1px solid rgba(103,232,249,.42);background:#07142d;box-shadow:0 18px 45px rgba(2,6,23,.45)}.dinosaur-paleoart svg{display:block;width:100%;height:auto;aspect-ratio:19/13}.dinosaur-paleoart figcaption{padding:.7rem 1rem;color:#bae6fd;background:linear-gradient(90deg,rgba(8,47,73,.95),rgba(15,23,42,.95));font-size:.84rem;letter-spacing:.02em}.fw-creature-card>.dinosaur-paleoart+span,.fw-creature-card>.dinosaur-paleoart+div:first-of-type{display:none}@media(max-width:640px){.dinosaur-paleoart{border-radius:1.2rem;margin-bottom:1rem}.dinosaur-paleoart figcaption{font-size:.76rem}}`;
  ...
  const art = (name) => {
    ...
    const safe = esc(name);
    const key = safe.replace(/\W/g,'');
    return `<figure class="dinosaur-paleoart" style="--body:${config.body};--accent:${config.accent}"><svg viewBox="0 0 760 520" role="img" aria-label="Scientific reconstruction of ${safe}"><defs><linearGradient id="sky-${key}" x1="0" y1="0" x2="0" y2="1"><stop stop-color="${config.sky}"/><stop offset="1" stop-color="#0f172a"/></linearGradient><linearGradient id="body-${key}" x1="0" y1="0" x2="1" y2="1"><stop stop-color="${config.accent}"/><stop offset=".35" stop-color="${config.body}"/><stop offset="1" stop-color="#111827"/></linearGradient><filter id="shadow-${key}"><feDropShadow dx="0" dy="16" stdDeviation="12" flood-opacity=".45"/></filter></defs><rect width="760" height="520" rx="32" fill="url(#sky-${key})"/><circle cx="620" cy="105" r="58" fill="#fef3c7" opacity=".75"/><path d="M0 377c110-72 204-51 309-11 126 48 270-21 451-49v203H0z" fill="${config.ground}"/><path d="M0 416c131-45 221-14 326 18 118 36 264-9 434-38v124H0z" fill="#0f172a" opacity=".65"/><g fill="url(#body-${key})" stroke="#020617" stroke-width="10" stroke-linejoin="round" filter="url(#shadow-${key})" style="--accent:${config.accent}">${pathFor(config.kind)}</g><text x="42" y="68" fill="#fff" font-size="34" font-family="Arial,sans-serif" font-weight="700">${safe}</text><text x="42" y="104" fill="#dbeafe" font-size="20" font-family="Arial,sans-serif">Paleoart reconstruction</text></svg><figcaption>${safe} · reconstructed prehistoric life</figcaption></figure>`;
  };
