...
if (fs.statSync(path.join(root, "public/assets/nico/photo/nico-photo-body.webp.b64")).size < 100_000) {
  throw new Error("The supplied high-resolution Nico body is incomplete");
}
...
if (!catalog.includes("WARDROBE_ITEM_BY_ID") || !catalog.includes("wardrobeForPreset") || !catalog.includes("itemsForSlot")) {
  throw new Error("Wardrobe lookup APIs are incomplete");
}

if (!photoBody.includes('PHOTO_NICO_WIDTH = 510') || !photoBody.includes('PHOTO_NICO_HEIGHT = 1467') || !photoBody.includes("nico-photo-body.webp.b64")) {
  throw new Error("The supplied Nico photo body dimensions or local path changed");
}
if (!photoBody.includes('data:image/webp;base64') || !photoBody.includes('cache: "force-cache"')) {
  throw new Error("The supplied Nico body is not loaded locally and cacheably");
}
if (!photoRenderer.includes('PHOTO_NICO_VIEWBOX') || !photoRenderer.includes('buildPhotoWardrobeBackgroundSvg') || !photoRenderer.includes('buildPhotoWardrobeForegroundSvg')) {
  throw new Error("Photo-calibrated clothing renderer is incomplete");
}
for (const slot of slots) if (!photoRenderer.includes(`data-photo-fit=\"${slot}`) && !photoRenderer.includes(`data-photo-fit="${slot}`)) {
  // Slot values are generated through wrap(item); verify the generic contract once and slot coverage through tests/catalog.
  if (!photoRenderer.includes('data-photo-fit="${item.slot}"')) throw new Error("Photo clothing layers lack fit metadata");
}
if (!photoRenderer.includes('PHOTO_BASE_ITEMS') || !photoRenderer.includes('nico-green-polo') || !photoRenderer.includes('nico-khaki-shorts')) {
  throw new Error("Photo renderer redraws or ignores the clothing already present on the supplied body");
}
if (!character.includes("usePhotoNicoBody") || !character.includes('data-photo-nico-body') || !character.includes("nico-photo-layer--body")) {
  throw new Error("The supplied Nico photo is not the primary wardrobe body");
}
if (!character.includes("wardrobeSvgDataUrl") || !character.includes("nico-vector-fallback")) {
  throw new Error("Local vector fallback was removed");
}
...
if (!drag.includes("setPointerCapture") || !drag.includes("elementFromPoint") || !drag.includes("data-nico-wardrobe-stage")) {
  throw new Error("Pointer/touch drag hit-testing is incomplete");
}
...
if (!studio.includes("onPointerDown") || !studio.includes("onPointerMove") || !studio.includes("onPointerUp")) {
  throw new Error("Wardrobe controls do not support pointer and touch dragging");
}
...
if (!showtime.includes("composeNicoImage(profile.nico.wardrobe)") || !showtime.includes("wardrobe={profile.nico.wardrobe}")) {
  throw new Error("Showtime live and recorded output do not share the wardrobe");
}
if (!compositor.includes("loadPhotoNicoBodyImage") || !compositor.includes("photoWardrobeBackgroundDataUrl") || !compositor.includes("photoWardrobeForegroundDataUrl")) {
  throw new Error("Showtime does not compose the supplied photo body and its clothing layers");
}
if (!compositor.includes("loadNicoWardrobeImage")) throw new Error("Showtime local fallback is missing");

for (const contract of [
  "data-photo-nico-body", "nico-photo-layer--back", "nico-photo-layer--body", "nico-photo-layer--front",
  "aspect-ratio:510/1467", "wardrobe-drag-ghost", "wardrobe-garment-grid", "touch-action:none", "prefers-reduced-motion",
]) if (!css.includes(contract)) throw new Error(`Wardrobe style contract missing: ${contract}`);

for (const contract of [
  "supplied Nico photo body", "dedicated 510 by 1467 coordinate system", "does not redraw clothing", "keeps backpacks behind",
  "renders the supplied image between transparent clothing layers", "retains the vector renderer as a local offline fallback",
  "equips one slot without replacing the other clothes", "supports remove, undo, and redo",
]) if (!tests.includes(contract)) throw new Error(`Wardrobe regression coverage missing: ${contract}`);

for (const [name, source] of [["catalog", catalog], ["reducer", reducer], ["drag", drag], ["character", character], ["studio", studio], ["photo renderer", photoRenderer]]) {
