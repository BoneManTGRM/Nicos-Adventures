...
test('wardrobe keeps approved art and supports favorite, undo, search and real PNG portrait',async({page},info)=>{
 ...
 await page.locator('.nico-dress-preview').scrollIntoViewIfNeeded();await shot(page,info,'wardrobe-portrait-studio');await bounds(page);await page.reload();await page.locator('[data-closet-category="favorites"]').click();await expect(cards).toHaveCount(1);expect(first).toBeTruthy();
});
test('movie director edits scenes and captions, saves exact timeline, and reopens without duplicate rewards',async({page},info)=>{
 ...
 await page.getByLabel(es?'Formato':'Format',{exact:true}).selectOption('portrait');await page.getByLabel(es?'Título':'Movie title',{exact:true}).fill('Three Friends');await page.locator('[data-movie-shot="1"]').click();await page.getByLabel(es?'Texto de esta toma':'This shot’s caption',{exact:true}).fill('A secret star for our team');
 ...
 await expect(page.getByLabel(es?'Formato':'Format',{exact:true})).toHaveValue('portrait');await page.locator('[data-movie-shot="1"]').click();await expect(page.getByLabel(es?'Texto de esta toma':'This shot’s caption',{exact:true})).toHaveValue('A secret star for our team');
});
test('movie recording creates real playable video and cancellation releases the capture stream',async({page},info)=>{
 ...
 await boot(page,info,'showtime');const es=language(info);await expect(page.getByTestId('movie-preview')).toBeEnabled({timeout:30000});
 await page.getByRole('button',{name:es?'Quitar toma':'Remove shot',exact:true}).click();await page.getByRole('button',{name:es?'Quitar toma':'Remove shot',exact:true}).click();await page.getByLabel(es?'Duración de toma':'Shot length',{exact:true}).selectOption('1000');
...
test('Monster Lab removes the ghost pod and the same monster plays a low-power action game',async({page},info)=>{
 ...
 const art=actualStage.locator('.monster-premium-body__art');await expect(art).toBeVisible();expect(await art.evaluate(node=>getComputedStyle(node).backgroundImage)).not.toBe('none');await shot(page,info,'monster-lab-no-glass-pod');
