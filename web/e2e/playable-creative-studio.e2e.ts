...
test('movie director edits scenes and captions, saves exact timeline, and reopens without duplicate rewards',async({page},info)=>{
 ...
 await page.getByRole('combobox',{name:es?'Formato':'Format',exact:true}).selectOption('portrait');await page.getByLabel(es?'Título':'Movie title',{exact:true}).fill('Three Friends');await page.locator('[data-movie-shot="1"]').click();await page.getByRole('textbox',{name:es?'Texto de esta toma':'This shot’s caption',exact:true}).fill('A secret star for our team');
 ...
 await expect(page.getByRole('combobox',{name:es?'Formato':'Format',exact:true})).toHaveValue('portrait');await page.locator('[data-movie-shot="1"]').click();await expect(page.getByRole('textbox',{name:es?'Texto de esta toma':'This shot’s caption',exact:true})).toHaveValue('A secret star for our team');
});
test('movie recording creates real playable video and cancellation releases the capture stream',async({page},info)=>{
 ...
 if(!supported){await expect(page.getByTestId('movie-record')).toBeDisabled();await expect(page.locator('.director-export')).toContainText(es?'no la grabación':'not recording');await info.attach('actual-recording-proof',{body:Buffer.from(JSON.stringify({realEncoder:false,capabilityFallback:true,reason:'No supported recording API/codec in this browser runtime'})),contentType:'application/json'});return;}
